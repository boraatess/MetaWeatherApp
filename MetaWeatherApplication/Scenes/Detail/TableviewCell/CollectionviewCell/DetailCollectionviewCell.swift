...
class DetailCollectionviewCell: UICollectionViewCell {
    ...
    private let labelPressure: UILabel = {
        let label = UILabel()
        label.font = .boldSystemFont(ofSize: 16)
        label.textColor = .black
        label.numberOfLines = 0
    ...
    private func layout() {
        ...
        labelHumidity.snp.makeConstraints { (maker) in
            maker.top.equalTo(self.imageCover.snp.bottom).offset(10)
            maker.trailing.equalToSuperview().inset(20)
        }
        addSubview(labelPressure)
        labelPressure.snp.makeConstraints { (maker) in
            maker.top.equalTo(self.imageCover.snp.bottom).offset(10)
            maker.leading.equalToSuperview().offset(20)
        }
    }
    ...
    func refreshWith(cityDetail: ConsolidatedWeather) {
        labelTitle.text = DetailViewController.name
        ...
        labelDate.text = cityDetail.applicableDate.formatDateString()
        ...
        labelHumidity.text = "Humidity \n"+String(cityDetail.humidity)
        labelPressure.text = "Air Pressure \n"+String(cityDetail.airPressure)
    }
